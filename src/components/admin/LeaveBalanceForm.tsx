...
export default function LeaveBalanceForm({
  ...
  const eligibleEmployees = employees.filter(
    (employee) => employee.leaveEligible,
  );
  ...
  const remaining = entitlement + carriedOver + adjusted - used;
  ...
  useEffect(() => {
    ...
    async function loadBalance() {
      ...
      try {
        ...
        if (!response.ok) {
          setError(data.error ?? "Gagal mengambil saldo cuti.");
        ...
        setAdjusted(data.balance.adjusted);

        setUsed(data.balance.used);
      } catch {
        if (!cancelled) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error ?? "Gagal menyimpan saldo cuti.");
      ...
      setSuccess("Saldo cuti berhasil disimpan.");
    } catch {
      setError("Terjadi masalah jaringan.");
    } finally {
      setSaving(false);
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl bg-white p-5 shadow-sm">
      <h2 className="text-lg font-semibold">Saldo Cuti</h2>
        ...
        {employeeId && (
          <>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-medium">Hak Cuti</label>
                ...
                <input
                  ...
                  value={entitlement}
                  onChange={(event) =>
                    setEntitlement(Number(event.target.value))
                  }
                  disabled={loading}
              ...
              <div>
                ...
                <input
                  ...
                  value={carriedOver}
                  onChange={(event) =>
                    setCarriedOver(Number(event.target.value))
                  }
                  disabled={loading}
                  className="mt-2 w-full rounded-xl border border-neutral-300 px-4 py-3 disabled:bg-neutral-100"
                />
            ...
            <div>
              ...
              <input
                type="number"
                value={adjusted}
                onChange={(event) => setAdjusted(Number(event.target.value))}
                disabled={loading}
                className="mt-2 w-full rounded-xl border border-neutral-300 px-4 py-3 disabled:bg-neutral-100"
              />

              <p className="mt-1 text-xs text-neutral-500">
                Bisa positif atau negatif. Contoh: 2 atau -1.
              </p>
            </div>
            ...
            <div className="rounded-xl bg-neutral-50 p-4">
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{carriedOver}</span>
                </div>
