...
export async function GET(
  ...
  if (
    event.photo.storageDisk !==
    "local"
  ) {
    return NextResponse.json(
      {
        error:
          "Storage foto tidak didukung.",
      },
      {
        status: 500,
      }
  ...
  try {
    ...
    return NextResponse.json(
      {
        error:
          "File foto tidak ditemukan di penyimpanan.",
      },
      {
        status: 404,
      }
  ...
  const fallbackFilename =
    `${event.employee.employeeCode}-${typeLabel}.jpg`;
